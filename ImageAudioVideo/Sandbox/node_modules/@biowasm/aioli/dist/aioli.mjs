const r = Symbol("Comlink.proxy"), g = Symbol("Comlink.endpoint"), x = Symbol("Comlink.releaseProxy"), o = Symbol("Comlink.thrown"), Y = (l) => typeof l == "object" && l !== null || typeof l == "function", k = {
  canHandle: (l) => Y(l) && l[r],
  serialize(l) {
    const { port1: c, port2: b } = new MessageChannel();
    return S(l, c), [b, [b]];
  },
  deserialize(l) {
    return l.start(), H(l);
  }
}, C = {
  canHandle: (l) => Y(l) && o in l,
  serialize({ value: l }) {
    let c;
    return l instanceof Error ? c = {
      isError: !0,
      value: {
        message: l.message,
        name: l.name,
        stack: l.stack
      }
    } : c = { isError: !1, value: l }, [c, []];
  },
  deserialize(l) {
    throw l.isError ? Object.assign(new Error(l.value.message), l.value) : l.value;
  }
}, R = /* @__PURE__ */ new Map([
  ["proxy", k],
  ["throw", C]
]);
function S(l, c = self) {
  c.addEventListener("message", function b(t) {
    if (!t || !t.data)
      return;
    const { id: i, type: e, path: m } = Object.assign({ path: [] }, t.data), d = (t.data.argumentList || []).map(u);
    let Z;
    try {
      const n = m.slice(0, -1).reduce((s, p) => s[p], l), a = m.reduce((s, p) => s[p], l);
      switch (e) {
        case "GET":
          Z = a;
          break;
        case "SET":
          n[m.slice(-1)[0]] = u(t.data.value), Z = !0;
          break;
        case "APPLY":
          Z = a.apply(n, d);
          break;
        case "CONSTRUCT":
          {
            const s = new a(...d);
            Z = U(s);
          }
          break;
        case "ENDPOINT":
          {
            const { port1: s, port2: p } = new MessageChannel();
            S(l, p), Z = T(s, [s]);
          }
          break;
        case "RELEASE":
          Z = void 0;
          break;
        default:
          return;
      }
    } catch (n) {
      Z = { value: n, [o]: 0 };
    }
    Promise.resolve(Z).catch((n) => ({ value: n, [o]: 0 })).then((n) => {
      const [a, s] = h(n);
      c.postMessage(Object.assign(Object.assign({}, a), { id: i }), s), e === "RELEASE" && (c.removeEventListener("message", b), V(c));
    });
  }), c.start && c.start();
}
function N(l) {
  return l.constructor.name === "MessagePort";
}
function V(l) {
  N(l) && l.close();
}
function H(l, c) {
  return X(l, [], c);
}
function W(l) {
  if (l)
    throw new Error("Proxy has been released and is not useable");
}
function X(l, c = [], b = function() {
}) {
  let t = !1;
  const i = new Proxy(b, {
    get(e, m) {
      if (W(t), m === x)
        return () => G(l, {
          type: "RELEASE",
          path: c.map((d) => d.toString())
        }).then(() => {
          V(l), t = !0;
        });
      if (m === "then") {
        if (c.length === 0)
          return { then: () => i };
        const d = G(l, {
          type: "GET",
          path: c.map((Z) => Z.toString())
        }).then(u);
        return d.then.bind(d);
      }
      return X(l, [...c, m]);
    },
    set(e, m, d) {
      W(t);
      const [Z, n] = h(d);
      return G(l, {
        type: "SET",
        path: [...c, m].map((a) => a.toString()),
        value: Z
      }, n).then(u);
    },
    apply(e, m, d) {
      W(t);
      const Z = c[c.length - 1];
      if (Z === g)
        return G(l, {
          type: "ENDPOINT"
        }).then(u);
      if (Z === "bind")
        return X(l, c.slice(0, -1));
      const [n, a] = y(d);
      return G(l, {
        type: "APPLY",
        path: c.map((s) => s.toString()),
        argumentList: n
      }, a).then(u);
    },
    construct(e, m) {
      W(t);
      const [d, Z] = y(m);
      return G(l, {
        type: "CONSTRUCT",
        path: c.map((n) => n.toString()),
        argumentList: d
      }, Z).then(u);
    }
  });
  return i;
}
function J(l) {
  return Array.prototype.concat.apply([], l);
}
function y(l) {
  const c = l.map(h);
  return [c.map((b) => b[0]), J(c.map((b) => b[1]))];
}
const I = /* @__PURE__ */ new WeakMap();
function T(l, c) {
  return I.set(l, c), l;
}
function U(l) {
  return Object.assign(l, { [r]: !0 });
}
function h(l) {
  for (const [c, b] of R)
    if (b.canHandle(l)) {
      const [t, i] = b.serialize(l);
      return [
        {
          type: "HANDLER",
          name: c,
          value: t
        },
        i
      ];
    }
  return [
    {
      type: "RAW",
      value: l
    },
    I.get(l) || []
  ];
}
function u(l) {
  switch (l.type) {
    case "HANDLER":
      return R.get(l.name).deserialize(l.value);
    case "RAW":
      return l.value;
  }
}
function G(l, c, b) {
  return new Promise((t) => {
    const i = z();
    l.addEventListener("message", function e(m) {
      !m.data || !m.data.id || m.data.id !== i || (l.removeEventListener("message", e), t(m.data));
    }), l.start && l.start(), l.postMessage(Object.assign({ id: i }, c), b);
  });
}
function z() {
  return new Array(4).fill(0).map(() => Math.floor(Math.random() * Number.MAX_SAFE_INTEGER).toString(16)).join("-");
}
const K = "", L = typeof window < "u" && window.Blob && new Blob([atob(K)], { type: "text/javascript;charset=utf-8" });
function v() {
  const l = L && (window.URL || window.webkitURL).createObjectURL(L);
  try {
    return l ? new Worker(l) : new Worker("data:application/javascript;base64," + K);
  } finally {
    l && (window.URL || window.webkitURL).revokeObjectURL(l);
  }
}
const F = "https://biowasm.com/cdn/v3", f = "https://stg.biowasm.com/cdn/v3", B = {
  urlCDN: F,
  urlCDNStg: f,
  dirShared: "/shared",
  dirMounted: "/mnt",
  dirData: "/data",
  printInterleaved: !0,
  printStream: !1,
  callback: null,
  debug: !1,
  env: "prd"
};
class w {
  constructor(c, b = {}) {
    if (c == null)
      throw "Expecting array of tools as input to Aioli constructor.";
    return Array.isArray(c) || (c = [c]), b = Object.assign({}, B, b), c = c.map(this._parseTool), b.env === "stg" && (b.urlCDN = b.urlCDNStg), this.tools = c, this.config = b, this.config.callback != null && (this.callback = this.config.callback), delete this.config.callback, this.init();
  }
  async init() {
    const c = new v();
    this.callback && (c.onmessage = (t) => {
      t.data.type === "biowasm" && this.callback(t.data.value);
    });
    const b = H(c);
    return b.tools = this.tools, b.config = this.config, await b.init(), b;
  }
  _parseTool(c) {
    if (typeof c != "string")
      return c;
    const b = c.split("/");
    if (b.length != 2 && b.length != 3)
      throw "Expecting '<tool>/<version>' or '<tool>/<program>/<version>'";
    return {
      tool: b[0],
      program: b.length == 3 ? b[1] : b[0],
      version: b[b.length - 1]
    };
  }
}
export {
  w as default
};
